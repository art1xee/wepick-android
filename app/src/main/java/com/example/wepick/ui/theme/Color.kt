package com.example.wepick.ui.theme

import androidx.compose.material3.ButtonColors
import androidx.compose.ui.graphics.Color


val PurpleGrey80 = Color(0xFFCCC2DC)

val Pink80 = Color(0xFFEFB8C8)

val Purple40 = Color(0xFF6650a4)
val PurpleGrey40 = Color(0xFF625b71)
val Pink40 = Color(0xFF7D5260)


// the main colors for every screens
val PrimaryPurple = Color(0xFF800E9C)
val DeepPurple = Color(0xFF3A1259)
val MidPurple= Color(0xFF5B1E86)
val AccentRed = Color(0xFFFF0037)
val Muted = Color(0xFF8B8B8B)

// В Color.kt
val TealSuccess = Color(0xFF1FA6A0)
val PinkAccent = Color(0xFFFF5C7A)
val ErrorRed = Color(0xFFE0435A)
val ProgressGray = Color(0x2EFFFFFF)

val CardYellow = Color(0xFFFFC840)
val CardYellowSoft = Color(0xFFFBDD8A)

val ButtonResetBg = Color(0xFFF8E10E)
val DarkButtonPurple = Color(0xFF2A1044)
val TextTeal = Color(0xFF015C65)
val InkSoft = Color(0xFF5C4A78)
val FieldBeige = Color(0xFFFFF6DD)
val FieldBorder = Color(0xFFE0AC2E)
val GoogleButtonBorder = Color(0xFF241436)

val White = Color(0xFFFFFFFF)
val Black = Color(0xFF000000)
val PinkLink = Color(0xFFFF5C7A)



// content type colors
val Movie = Color(0xffa87b00)
val Series = Color(0xff39aecf)
val Anime = Color(0xffdb62c4)

//partner type color
val FriendColor = Color(0xFFE1FA71)
val PopularCharacterColor = Color(0xFF62AA2A)
val DisabledContentButton = Color(0xFFDDDDDD)

// choose genres colors
val borderDislikes = Color(0xffff4d4d)
val contentDislikes = Color(0xffff4d4d)
val borderDislikesShadow = Color(0xffff4d4d)
val borderLikes = Color(0xff4d79ff)
val contentLikes = Color(0xff4d79ff)
val borderLikesShadow = Color(0xff4d79ff)

// summary colors
val DislikeContentColor = Color(0xffff6b6b)
val LikeContentColor = Color(0x3344FF44)
val DecadeColor = Color(0x334444FF)
